...
export function buildSystemPrompt(options: {
  ...
  const queryGuidelines = [
    ...
    toolsSection.includes('visualize_data')
      ? '- When a chart would clarify a result, use visualize_data and present the returned spec in a code block with the language "chart".'
      : '',
    toolsSection.includes('export_report')
      ? '- Use export_report to compile findings into a structured report when the user asks for a summary or analysis.'
      : '',
    toolsSection.includes('search_memory')
  ...
  const chartGuidelines =
    '- If the user explicitly asks for a chart, graph, plot, or visualization, run the relevant query, call visualize_data once with the returned columns and rows, and include the returned spec in a ```chart code block. Do not answer an explicit visualization request with only a table.\n' +
    '- For analytical results with four or more rows and a clear category/time column plus one or more numeric columns, decide whether one chart would make the comparison or trend easier to understand. Skip charts for single values, raw detail lists, or results without a meaningful visual relationship.\n' +
    '- Use at most one chart per answer unless the user asks for multiple views.\n' +
    '- Call visualize_data with your query columns/rows, chart type, and optionally nameKey/valueKeys.\n' +
    '- The tool supports: bar (vertical), line (connected points), area (filled line), pie (circular segments), scatter (xy points), radar (spider web), radialBar (circular bars), composed (bar+line+area mix), funnel (progressive stages), treemap (nested boxes), sunburst (ring hierarchy), slope (two-endpoint comparison).\n' +
    ...
    '- For slope charts, pass exactly two valueKeys and use options.startLabel/options.endLabel to name the endpoints.\n' +
    '- Use options.showValueLabels for direct values and sparse annotations for meaningful callouts, reference lines, or highlighted ranges; avoid annotating every mark.\n' +
    '- Present the returned chart data in a ```chart code block, never as bare JSON in prose:\n\n' +
    '  ```chart\n' +
