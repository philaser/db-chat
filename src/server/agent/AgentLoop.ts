import type { EffortLevel, ChatMessage, ModelChatMessage, AgentEvent, AgentToolResult, QueryResultArtifact } from '../../shared/types.js';
...
export async function runAgentLoop(
  ...
  let previousRoundStreamedContent = false;
  ...
  try {
    for (let round = 0; round < maxTurnRounds; round++) {
      ...
      if (round > 0 && previousRoundStreamedContent) {
        activity.textDelta('\n\n');
      }
      ...
      previousRoundStreamedContent = modelResponse.content.trim().length > 0;
      ...
      for (const call of pendingCalls) {
        ...
        if (permission === 'ask') {
          if (config.approvalManager.isAllowedBySession(toolName, input)) {
            await executeToolAndRecord(toolName, input, call.id, toolContext, config.toolRegistry, activity, conversation, allToolCalls, artifacts);
            totalToolCalls++;
            continue;
          }
    ...
    return buildResult(activity, allToolCalls, artifacts, 'Analysis complete.');
  } catch (error) {
...
async function executeToolAndRecord(
  ...
  if (artifact && toolName === 'run_database_query') {
    const queryArtifact: QueryResultArtifact = {
      kind: 'query-result',
      queryId: `${toolContext.turnId}-query-${artifacts.length + 1}`,
