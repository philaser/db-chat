...
export function findBlockedAggregationStage(pipeline: unknown[]): string | null {
  function visit(value: unknown): string | null {
    if (Array.isArray(value)) {
      for (const child of value) { const blocked = visit(child); if (blocked) return blocked; }
    } else if (isRecord(value)) {
      for (const [key, child] of Object.entries(value)) {
        if (BLOCKED_AGGREGATION_STAGES.has(key)) return key;
        const blocked = visit(child); if (blocked) return blocked;
      }
    }
    return null;
  }
