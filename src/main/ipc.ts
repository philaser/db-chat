  ...
  async sendChat(messages: ModelChatMessage[]) {
    ...
    if (!apiKey) {
      ...
      const queryResult = local.query && this.connector && local.query.validation.safe
        ? await this.connector.executeQuery(local.query.query)
        : undefined;
      return {
        message: {
          id: crypto.randomUUID(),
    ...
    const content = await provider.sendChat([
      { role: 'system', content: buildSystemPrompt(schemaContext, this.schema?.kind ?? 'sqlite') },
      ...chatHistory
    ], {
    ...
    const queryResult = generatedQuery?.validation.safe && this.connector
      ? await this.connector.executeQuery(generatedQuery.query)
      : undefined;
    const responseContent = queryResult && generatedQuery
      ...
      : generatedQuery && !generatedQuery.validation.safe
        ? `I found a query for that, but SAFE mode did not run it: ${generatedQuery.validation.reason}`
  ...
  private async buildResultAwareResponse(
    ...
    try {
      const content = await provider.sendChat([
        {
          role: 'system',
          content: buildResultAnalysisPrompt(this.schema?.kind ?? 'sqlite')
        },
        ...chatHistory,
        {
          role: 'assistant',
