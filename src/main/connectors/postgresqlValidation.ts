...
const READ_ONLY_PREFIXES = ['select', 'with', 'show', 'explain'];
const MANUAL_PREFIXES = [...READ_ONLY_PREFIXES, 'insert', 'update', 'delete'];
...
export function normalizePostgresqlQuery(query: string): string {
  return stripSqlComments(query).replace(/\s+/g, ' ').trim();
}

export function validatePostgresqlReadOnlyQuery(query: string, mode: QueryExecutionMode): QueryValidationResult {
  const normalizedQuery = normalizePostgresqlQuery(query);
  ...
  if (!prefixes.some((prefix) => lower.startsWith(prefix))) {
    return {
      safe: false,
      reason: mode === 'safe'
        ? 'SAFE mode only allows SELECT, WITH, SHOW, and EXPLAIN.'
